...
- [`WORKFLOW.md`](WORKFLOW.md) — authoritative, version-agnostic process doc: the per-version workflow (plan → branch → implement → gate → log → release) and standing instructions for Claude Code. Read it before starting any new version/feature.
...
Versioning: pre-1.0, one read-endpoint group per minor. For the current objective and backlog see [`ROADMAP.md`](ROADMAP.md).
